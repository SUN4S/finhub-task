...
const fileRotateTransportWarn = new winston.transports.DailyRotateFile({
  ...
  maxFiles: "14d",
  level: "warn",
  format: winston.format.combine(warnFilter(), timestamp({ format: "YYYY-MM-DD hh:mm:ss" }), json())
});
...
const fileRotateTransportInfo = new winston.transports.DailyRotateFile({
  ...
  maxFiles: "14d",
  level: "info",
  format: winston.format.combine(infoFilter(), timestamp({ format: "YYYY-MM-DD hh:mm:ss" }), json())
});
...
const fileRotateTransportHttp = new winston.transports.DailyRotateFile({
  ...
  maxFiles: "14d",
  level: "http",
  format: winston.format.combine(httpFilter(), timestamp({ format: "YYYY-MM-DD hh:mm:ss" }), json())
});
fileRotateTransportInfo.on("rotate", (oldFilename, newFilename) => {
  console.log("You should probably save this somewhere");
});
...
export const logger = winston.createLogger({
  ...
  exceptionHandlers: [
    new winston.transports.File({
      filename: path.resolve(process.cwd() + "/logs/exception.log")
    })
  ],
  rejectionHandlers: [
    new winston.transports.File({
      filename: path.resolve(process.cwd() + "/logs/rejections.log")
    })
  ]
});
